...
![версия](https://img.shields.io/badge/version-2.5.33-3b5bdb)
...
<img src="docs/ui-whoami.ru.png?v=2.5.33" width="100%" alt="Страница Who Am I: хеш отпечатка, платформа, язык и часовой пояс, затем выдуманное оборудование, экран, видеокарта, состояние canvas и WebRTC и список активных модулей">
...
<img src="docs/ui-popup.ru.png?v=2.5.33" width="100%" alt="Попап расширения: защита активна, шесть из шести модулей, страна выхода, посайтовые переключатели WebRTC, Service Worker и CSP, выбор обычного или скрытого режима и профиль устройства">
...
<img src="docs/ui-modules.ru.png?v=2.5.33" width="100%" alt="Сетка модулей защиты в настройках: Canvas, WebGL, WebRTC, Navigator, Screen, Timezone, Geolocation, Battery, Fonts, ClientRects, Plugins, Network и Hide AdBlock — каждый галочкой с однострочным описанием">
...
17. **Маркер воркера фиксирован, а канал окна — константа.** `__AFP_PATCH_URL` — глобал
    скоупа воркера: страница не читает его напрямую, но созданный ею воркер читает, и там
    он отвечает за эту сборку, тогда как у чистого браузера нет ничего. На окне имени больше
    нет вовсе. Набор статуса был `'__t0' in window` — одна строка, без всякой базы для
    сравнения, и у чистого браузера «нет» на любом origin; теперь его выдаёт слушатель
    фиксированного типа события, поэтому дифф собственных имён против чистого браузера пуст,
    а перечислить слушатели платформа не даёт. Остаётся то, что тип — константа в исходниках
    расширения: детектор, прочитавший исходники, может отправить событие и получить тот же
    объект. Вывести тип из сида мешает то же, что мешало вывести старые имена: канал нужен
    раньше, чем известен сид, и его читают проверки в живом браузере.
